...
function DragModule.MakeDraggable(frame, handle)
    ...
    local dragStart, startPos, activeInput -- Added activeInput to lock the finger
    ...
    handle.InputBegan:Connect(function(input)
        if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and not dragToggle then
            ...
            activeInput = input -- Lock this specific finger/click
            
            -- Disconnect when this specific finger lifts up
            local connection
    ...
    UIS.InputChanged:Connect(function(input)
        -- ONLY move if the input changing is the one we locked in InputBegan
